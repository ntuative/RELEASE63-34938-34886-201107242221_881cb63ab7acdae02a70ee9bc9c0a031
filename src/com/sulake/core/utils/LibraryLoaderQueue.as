package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1949:int = 4;
      ...
      {
         return this.var_478.length + this.var_314.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_314.length < const_1949 && this.var_478.length > 0)
            {
               _loc1_ = this.var_478.shift();
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_478.indexOf(loader);
            if(index > -1)
            {
               this.var_478.splice(index,1);
            }
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_478)
            {
               if(param2)
               {
