package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private const const_2384:int = 13421772;
      
      private const const_2383:int = 16777215;
      
      private const const_394:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this._colors = param4;
         this.var_2059 = param5;
         if(param3 == null)
         {
            this.var_761 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_2060 = Math.max(this.var_2060,_loc6_.colorLayerIndex);
            }
         }
         this.var_485 = this.var_78.controller.manager.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_761 != null && !this.var_2059)
            {
               _loc3_ = this.var_761;
            }
            else
            ...
            if(this.var_875)
            {
               switch(this.var_875.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_35:
                     _loc2_.style = HabboIconType.const_754;
                     break;
         ...
         if(this.isSelected)
         {
            this.var_1549.color = this.const_2384;
         }
         else
         {
            this.var_1549.color = this.const_2383;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_485.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_485.isFigureReady(_loc1_))
         {
            this.var_485.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_394[_loc6_] + "_" + FigureData.const_163;
               _loc4_ = this.var_485.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_394.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_394[_loc6_] + "_" + FigureData.const_163;
                  _loc4_ = this.var_485.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         _loc1_ = new BitmapData(this.var_710.width,this.var_710.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_710.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_710.y;
               if(this.var_2059 && _loc2_.colorLayerIndex > 0)
               {
