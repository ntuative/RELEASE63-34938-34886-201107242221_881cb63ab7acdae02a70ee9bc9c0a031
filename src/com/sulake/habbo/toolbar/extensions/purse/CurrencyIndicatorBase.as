package com.sulake.habbo.toolbar.extensions.purse
{
   ...
   public class CurrencyIndicatorBase implements ICurrencyIndicator
   {
      ...
      private var var_302:Vector.<BitmapData>;
      
      private var var_2897:uint;
      
      private var var_2896:String = null;
      
      private var var_2117:uint = 0;
      ...
      {
         this.var_302 = new Vector.<BitmapData>();
         super();
         this._view = null;
      ...
      protected function createWindow(param1:String, param2:String) : void
      {
         ...
         if(_loc3_)
         {
            ...
            if(this._view)
            {
               ...
               {
                  this.var_723 = _loc4_[0] as IBitmapWrapperWindow;
      ...
      protected function animateIcon(param1:int) : void
      {
         ...
         if(this.var_723 && this.var_302.length > 0)
         {
            if(this.var_2117 == ANIM_DIRECTION_FORWARD)
            {
               this.var_303 = 0;
            }
            else
            {
               this.var_303 = this.var_302.length - 1;
            }
            this.var_722 = new Timer(this.var_2897,this.var_302.length);
      ...
      protected function setText(param1:String) : void
      {
         if(this._view)
         {
            this._view.findChildByName(this.var_2119).caption = param1;
            this._view.findChildByName(this.var_2118).caption = param1;
         }
      }
      ...
      protected function setTextUnderline(param1:Boolean) : void
      {
         ...
         if(this._view)
         {
            _loc2_ = this._view.findChildByName(this.var_2119) as ITextWindow;
            _loc3_ = this._view.findChildByName(this.var_2118) as ITextWindow;
      ...
      protected function set iconAnimationSequence(param1:Array) : void
      {
         ...
         for each(_loc2_ in param1)
         {
            ...
            if(_loc3_)
            {
               ...
               if(_loc4_)
               {
                  this.var_302.push(_loc4_.clone());
               }
            }
         }
      }
      ...
      private function onAnimationTimer(param1:TimerEvent) : void
      {
         if(this.var_723 && this.var_302.length > 0)
         {
            this.setIconBitmap(this.var_302[this.var_303]);
            if(this.var_2117 == ANIM_DIRECTION_FORWARD)
            {
               ++this.var_303;
               this.var_303 = this.var_303 < this.var_302.length ? int(this.var_303) : int(this.var_302.length - 1);
            }
            else
            {
               --this.var_303;
      ...
      private function onAnimationTimerComplete(param1:TimerEvent) : void
      {
         ...
         {
            this.setIconBitmap(this.var_302[0]);
         }
      }
      ...
      private function setIconBitmap(param1:BitmapData) : void
      {
         if(this.var_723)
         {
            this.var_723.bitmap = param1.clone();
            this.var_723.width = param1.width;
            this.var_723.height = param1.height;
            this.var_723.invalidate();
         }
      }
      
      private function onContainerMouseOver(param1:WindowMouseEvent) : void
      {
         this._view.color = this.var_2895;
      }
      
      private function onContainerMouseOut(param1:WindowMouseEvent) : void
      {
         this._view.color = this.var_2898;
      }
   }
}
