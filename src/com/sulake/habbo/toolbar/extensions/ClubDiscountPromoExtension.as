package com.sulake.habbo.toolbar.extensions
{
   ...
   public class ClubDiscountPromoExtension
   {
      ...
      private static const const_682:String = "club_promo";
      
      private static const const_1440:int = 13;
      
      private static const const_1047:int = 14;
      
      private static const const_1439:uint = 16777215;
      ...
      private var var_620:IExtensionView;
      ...
      public function ClubDiscountPromoExtension(param1:HabboToolbar, param2:IHabboWindowManager, param3:IAssetLibrary, param4:IEventDispatcher, param5:IHabboConfigurationManager, param6:ICoreLocalizationManager, param7:IHabboInventory, param8:IHabboCatalog, param9:IConnection)
      {
         ...
         this.var_620 = param1.extensionView;
         if(this.var_12)
         {
            this.var_12.events.addEventListener(HabboInventoryHabboClubEvent.const_324,this.onClubChanged);
         }
      }
         ...
         if(_loc4_)
         {
            ...
            if(_loc3_)
            {
               ...
               if(this.var_83)
               {
                  ...
                  if(_loc5_)
                  {
                     this.var_770 = _loc5_.content as BitmapData;
                     if(this.var_770)
                     {
                        this.var_83.bitmap = this.var_770.clone();
                     }
                  }
                  this.var_83.visible = false;
               }
               this.var_1173 = _loc3_.findChildByName("text_region") as IRegionWindow;
               if(this.var_1173)
               {
                  this.var_1173.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK,this.onTextRegionClicked);
                  this.var_1173.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_OVER,this.onTextRegionMouseOver);
                  this.var_1173.addEventListener(WindowMouseEvent.const_25,this.onTextRegionMouseOut);
               }
               this.assignState();
            }
         }
      ...
      public function dispose() : void
      {
         if(!this._disposed)
         {
            if(this.var_620)
            {
               this.var_620.detachExtension(const_682);
               this.var_620 = null;
            }
            this.animate(false);
            this.clearAnimation();
            ...
            if(this.var_12)
            {
               this.var_12.events.removeEventListener(HabboInventoryHabboClubEvent.const_324,this.onClubChanged);
               this.var_12 = null;
            }
            this._catalog = null;
      ...
      private function onClubChanged(param1:HabboInventoryHabboClubEvent) : void
      {
         if(this.var_12.clubIsExpiring)
         {
            if(!this._view)
            {
               ...
               this.assignState();
               this.var_620.attachExtension(const_682,this._view,ExtensionFixedSlotsEnum.const_2021);
            }
         }
         else
         {
      ...
      private function animate(param1:Boolean) : void
      {
         if(param1)
         {
            ...
            this.var_628 = new Timer(15000);
            this.var_628.addEventListener(TimerEvent.TIMER,this.onTriggerTimer);
            this.var_628.start();
         }
         else
         {
      ...
      private function resetAnimationVariables() : void
      {
         ...
         this.var_1744 = this._view.width - 7 - this.var_83.bitmap.width;
         this.var_1446 = 0;
      }
      ...
      private function onAnimationTimer(param1:TimerEvent) : void
      {
         var _loc2_:int = 0;
         var _loc3_:* = null;
         this.var_83.x = 3 + this.var_1446 / 20 * this.var_1744;
         if(this.var_83.x > this.var_1744)
         {
            _loc2_ = this._view.width - 4 - this.var_83.x;
            _loc3_ = new BitmapData(_loc2_,this.var_770.height);
            _loc3_.copyPixels(this.var_770,new Rectangle(0,0,_loc2_,this.var_770.height),new Point(0,0));
