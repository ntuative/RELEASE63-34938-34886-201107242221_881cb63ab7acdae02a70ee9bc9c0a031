package com.sulake.habbo.ui.widget.effects
{
   ...
   public class EffectsWidget extends RoomWidgetBase
   {
      ...
      private static const const_1693:int = 320;
      
      private static const const_1694:int = 48;
      
      private static const const_1692:int = 2;
      ...
      public function open() : void
      {
         ...
         if(!this._view)
         {
            ...
            this._view.x = _loc2_.right + const_1692;
      ...
      public function update() : void
      {
         ...
         {
            _loc1_ = this.var_414.getValue(_loc3_.type) as EffectView;
            if(_loc1_)
            {
               _loc1_.update();
            }
            else
            {
               ...
               this.var_414.add(_loc3_.type,_loc1_);
               this.var_66.addListItem(_loc1_.window);
            }
         }
         _loc4_ = this.var_414.length - 1;
         while(_loc4_ >= 0)
         {
            _loc1_ = this.var_414.getWithIndex(_loc4_);
            if(_loc2_.indexOf(_loc1_.effect) == -1)
            {
               this.var_66.removeListItem(_loc1_.window);
               this.var_414.remove(this.var_414.getKey(_loc4_));
               _loc1_.dispose();
            }
            _loc4_--;
         }
         var _loc5_:int = this.var_66.scrollableRegion.height;
         this.var_66.height = Math.max(Math.min(_loc5_,const_1693),const_1694);
      ...
      {
         this.handler.container.catalog.openCatalogPage(CatalogPageName.const_1270,true);
      }
   }
}
